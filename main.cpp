...
public:
	PolygonObject() {}
	PolygonObject(const olc::vf2d& _position, float _angle, float _len, int _n_vertices)
		: position(_position), angle(_angle), len(_len), n_vertices(_n_vertices) {
...
public:
	...
	void OnMouseReleaseAdd() {
		...
		PolygonObject polygon(position, angle, len, n_vertices);
		polygon.model = model;
