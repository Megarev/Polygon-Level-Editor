...
public:
    ...
    void Draw(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge, bool is_fill = false, float alpha = 0.0f); // Polygon Line Drawing
    void Draw(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge, float alpha = 0.0f); // Decal drawing
...
public:
    ...
    void SetBounds(const olc::vf2d& _screen_size) { screen_size = _screen_size; }
    void Initialize(const olc::vf2d& _screen_size) { SetBounds(_screen_size); }

    void Update(float dt, bool is_debug = false);
    void Draw(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge, bool is_fill = false);
    void DrawDecals(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge);
    ...
    void AddShape(const RigidBody& rb) { shapes.push_back(rb); shapes.back().SetID(shapes.size()); }
...
RigidBody::RigidBody(const olc::vf2d& p, int _n, float _len, float _angle, float a, float m, float _e, float _sf, float _df, int _id)
    ...
    for (int i = 0; i < n; i++) {
        model.push_back({ cosf(2.0f * PI / n * i + PI / 4.0f), sinf(2.0f * PI / n * i + PI / 4.0f) });
...
void RigidBody::Draw(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge, bool is_fill, float alpha) {
    ...
    for (uint32_t a = 0; a < vertices.size(); a++) {
        ...
        interpolated_vertices[a] += pan_offset;
    }
...
void RigidBody::Draw(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge, float alpha) {
...
void Manifold::PositionalCorrection() {
    float p = 0.5f;
    const olc::vf2d& direction = std::fmaxf(overlap - 0.025f, 0.0f) * normal * p / (a->mass + b->mass);
...
void Scene::Draw(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge, bool is_fill) {
    for (auto& s : shapes) s.Draw(pan_offset, pge, is_fill, 0.0f);
    for (auto& c : constraints) c.Draw(pge);
}

void Scene::DrawDecals(const olc::vf2d& pan_offset, olc::PixelGameEngine* pge) {
    for (auto& s : shapes) s.Draw(pan_offset, pge, 0.0f);
    for (auto& c : constraints) c.Draw(pge);
}
